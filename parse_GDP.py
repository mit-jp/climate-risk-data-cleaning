...
for i in range(df_virginia.shape[0]):
    df_virginia.iloc[i, 8] = ref_virginia.iloc[i, 0]
    df_virginia.iloc[i, 9] = ref_virginia.iloc[i, 1]
df_virginia = df_virginia.sort_values(by = ['STATEFP', 'COUNTYFP'], axis=0) #resort
